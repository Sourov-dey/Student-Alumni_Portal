  ...
  return (
    <div style={{ maxWidth: 900, margin: '20px auto' }}>
      ...
      <div style={{ marginTop: 16 }}>
        ...
        {notifications.map(n => (
          <div key={n.id} style={{ padding: 12, border: '1px solid #eee', borderRadius: 8, marginBottom: 10, background: n.read ? '#fff' : '#f8fbff' }}>
            <div>{n.message}</div>
            <div style={{ color:'#666', fontSize: 13, marginTop: 6 }}>{n.type} • {new Date(n.createdAt).toLocaleString()}</div>
            <div style={{ marginTop: 8 }}>
              <button className="btn small" onClick={() => markAsRead(n.id)}>Mark read</button>
